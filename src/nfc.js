class NFC {
  ...
  async session() {
    ...
    // ferica poling
    await this.send([0xFF, 0x50, 0x00, 0x01, 0x00, 0x00, 0x11, 0x5F, 0x46, 0x04, 0xA0, 0x86, 0x01, 0x00, 0x95, 0x82, 0x00, 0x06, 0x06, 0x00, 0xFF, 0xFF, 0x01, 0x00, 0x00, 0x00, 0x00]);
    ...
    // SwitchProtocolTypeA
    await this.send([0xff, 0x50, 0x00, 0x02, 0x04, 0x8f, 0x02, 0x00, 0x03, 0x00]);
    await this.receive(len);    
    // GET Card UID
  ...
  async connectUSBDevice() {
    ...
    try {
      ...
      // 自動選択 or 選択画面
      this.device = pearedDevices.length == 1 ? pearedDevices[0] : await navigator.usb.requestDevice({ filters: deviceFilters});
      this.cL("open");
      ...
      this.cL(this.device);
      const deviceInterface = this.device.configuration.interfaces.filter(v => v.alternate.interfaceClass == 255)[0];
